		...
		public String getTitle(String code) {
		   ...
		   //가져온 코드를 통해 아이디 가져오기 (가연 추가)
		      ...
		      public ArrayList<CoseDTO> getOneCoseByCode(String code){
		         ...
		         String sql = "select * from cose_table where code = ? order by day asc, starttime asc";
...
//---------------관리자 댓글 수정/삭제(가연)
			    //admin 계정으로 댓글 수정이나 삭제 할 때 수정(삭제)할 댓글을 가져오는 메소드(가연 추가)
					...
					//관리자 코스 삭제 메소드
					public int AdminDeleteCose(String code)
					{
						Connection conn = null;
						PreparedStatement pstmt = null;
						int result = -1;
						
						String sql = "delete from cose_code_table where code= ?";
		             ...
		             public int rootUpdate(CoseDTO rdto) {
		                 int result=-1;
		                 String sql = "update cose_table set place_name=?, img=?, addr=?,price=? where cose_date=? and starttime=? and code=?";
		                 Connection conn = null;
		                   PreparedStatement pstmt = null;
		                   System.out.println(rdto);
		                   java.util.Date utilDate = rdto.getCose_date();
		                   ...
		                   try {
		                         conn = DBManager.getConnection();
		                         pstmt = conn.prepareStatement(sql);
		                         pstmt.setString(1, rdto.getPlaceName());
		                         pstmt.setString(2, rdto.getImg());
		                         pstmt.setString(3, rdto.getAddr());
		                         pstmt.setString(4, rdto.getPrice());
		                         pstmt.setDate(5, sqlDate);
		                         pstmt.setString(6, rdto.getStarttime());
		                         pstmt.setString(7, rdto.getCode());
		              ...
		              public int update_content(String code,String content) {
		                 int result=-1;
		                 String sql = "update cose_table set content=? where code=?";
		                 Connection conn = null;
		                   PreparedStatement pstmt = null;
		                   try {
		                         conn = DBManager.getConnection();
