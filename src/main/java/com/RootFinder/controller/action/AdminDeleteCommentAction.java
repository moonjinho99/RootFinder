...
public class AdminDeleteCommentAction implements Action{
@Override
...
CoseDAO cDao = CoseDAO.getInstance();
	...
	int result =  cDao.admin_delete_comment(code,prt_num,cdr_num);
	
	
	if(result == 1)
	{
		System.out.println("관리자_댓글 삭제 성공");
	} else {
		System.out.println("관리자_댓글 삭제 실패");
	}
