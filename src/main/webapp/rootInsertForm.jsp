...
<meta charset="UTF-8">
<title>코스 등록</title>
<link rel="stylesheet" href="./css/rootinsert.css" />
...
<body>
	<form action="RootFinderServlet?command=root_insert" method="post"
		name="frm" enctype="multipart/form-data">
		<h2><%=lastday%>일 간의 여행 어떠셨나요? 일정에 대한 자세한 설명을 적어주세요
		</h2>
		<hr>
		<textarea name="content" rows="20" cols="80"></textarea><br>
		<input type="hidden" name="id" value="${loginUser.id}"> <input
			type="hidden" name="pwd" value="${loginUser.pwd}"> 코스를 대표할
		이미지를 등록해 주세요! <input type="file" name="main_img"> <br>
		코스의 대표 주소를 입력해 주세요!<input type="text" id="sample6_address"
			name="main_addr" placeholder="주소"> <input type="text"
			id="sample6_extraAddress" placeholder="참고항목"> <input
			type="button" onclick="sample6_execDaumPostcode()" value="주소 찾기"><br>
